...
<?php
	  ...
	  DrawCalendarHelp();
       function DrawCalendarHelp(){
		     global $Show;
	         if($Show!="Help")return;
			 DrawHelp("Help/CalendarHelp.png",100,100,1000,500,"Calendar.php");
	   }
	   function DrawProject(){
		     ...
		     DrawRect(" ","10","#ffffff", $StartX ,$y,( $StartX-20+( count($Projects)+1)*60+40),"20","#000000");
		   	 for($i=0;$i<count($Projects);$i++){
				 ...
				 DrawRect( $Projects[$i][0],"10","#ffffff", $x,$y+5, "58","12",$color);
			 }
			...
			// DrawLinkPic("Pics/Logout.png",$startY+12,150,14,14"Login.php");
			 //Help/CalendarHelp.png
	   }
       function DrawMembers(){
			 ...
			 for($i=0;$i<count($members);$i++){
				 $x= $CalendarWidth-50- ($i)*60;
				 $color="#000000";
			     DrawMemberRect($members[$i][1],"11","#ffffff", $x, "25", "60","40",$color,$members[$i][4]);
	   ...
	   function DrawUsers($y,$m){
	    ...
	    function DrawEdit(){
		    ...
		    $seletProject=MakeSelection($Projects,0,$CProject,"Project");
		    DrawInputRect( "專案","14","#ffffff" ,$x+460,$y+80,120,20, $colorCodes[4][2],"top", $seletProject);
	   ...
	   function MakeSelection($items,$arrayNum,$selectItem,$selectName){
		   echo $selectName;
	        $seletProject= "<select name=".$selectName.">";
	   ...
	   function getDivHeight($Data_Struct){
		   	...
		   	for ($i=0;$i<count( $Data_Struct);$i++){
			     ...
			     $str= floor((strlen( $info )/6)/ $workDays) ;
				 //echo"$info=".(strlen( $info )/3);
				 //if($str>1)echo"$info=".(strlen( $info )).">".$str;
				 if( $str>$h)$h=$str;
			}
			// echo ">>".$h;
			return $h;
	   ...
	   function DrawUsersWorks($y,$m,$ID){
		    	...
		    	for ($i=0;$i<count( $Data_Struct);$i++){
					...
					//echo "[".$i.$Data_Struct[$i][0]."fd:".$finDay."]";
			     }
				 //溢月資料
	   ...
	   function getUserWorks($y,$m,$user){
			   ...
			   for ($i=0;$i<$t;$i++){
				    ...
				    if($dy==$y   And $dm==$m  And $duser==$user){
						 //檢查跨月
						 $MonthEnd= getMonthDay($m,$y);
					     $workDays= ReturnWorkDays($dm,$dstartDay, $dWorkDays);
						...
						//  echo "[".$i.$dinfo."sd:".$dstartDay."-".$dWorkDays."fix".$workDays."fd:".$finDay."]";
					     ...
					     if($finDay>$MonthEnd){
							  ...
							  $nextM=$m+1;
							  if($nextM>12)$nextM=1;
                              $CrossFinDays=ReturnWorkDays($nextM,1,  $CrossDays);
       ...
       function ReturnWorkDays($m,$startday,$workd){
		  		global  $VacationDays;
				$AccumulatekDays=0;
				$CurrentDay=$startyday;
				while($AccumulatekDays<$workd){ 
			
			       if(!in_array(array($m,$CurrentDay),$VacationDays )){
					 $AccumulatekDays+=1;
				   }
				  	  $CurrentDay+=1;
